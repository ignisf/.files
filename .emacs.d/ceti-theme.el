;;; ceti-theme.el --- A Hyperminimal UI Theme

;; Copyright (C) 2013 Petko Bordjukov
...
;; Bruce Williams for the spacegray theme
...
(deftheme ceti
  "A Hyperminimal UI Theme for Emacs based on spacegray")
(display-color-cells (selected-frame))
(let* ((class '((class color) (min-colors 89)))
       (256color (eq (display-color-cells (selected-frame)) 256))
       (background "#343a41") ;; sidebar-container
  ...
  (custom-theme-set-faces
   ...
   `(font-lock-type-face ((,class (:foreground ,green :slant italic))))
   `(font-lock-variable-name-face ((,class (:foreground ,red :slant italic))))
   ...
   `(ido-first-match ((,class (:foreground ,purple))))
   ...
   `(border ((,class (:background ,far-background))))
   `(vertical-border ((,class (:foreground ,far-background :background ,background))))
   ...
   `(mode-line ((,class (:foreground ,foreground :background ,background
                                     :box (:line-width -1 :color ,far-background :style nil)))))
   ...
   `(diff-hl-change ((,class (:foreground ,purple))))
